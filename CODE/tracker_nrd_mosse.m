function [rect_results, time] = tracker_nrd_mosse(video_path, img_files, pos, target_sz, ...
...
%     w2c = temp.w2crs;
	...
	for frame = 1:numel(img_files)
		...
		if resize_image
			im = imresize(im, 0.5);
%             im = mexResize(im, 0.5*[w h], 'linear');
        end
		...
		if frame > 1
            for i=1:size(search_size, 2)
                ...
                z = get_features(patch, features, cell_size, cos_window, w2c, w, h, channel);
                zf = fft2(z);
        ...
        patch = get_subwindow_mhy(im, pos, tmp_sz, window_sz);
		x_pca = get_features(patch, features, cell_size, cos_window,w2c, w, h, channel);
