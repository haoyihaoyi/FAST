function [rect_results, time] = tracker_good_mosse_interp(video_path, img_files, pos, target_sz, ...
    ...
    responsef = zeros(w,h,size(search_size,2));
...
%             im = mexResize(im, 0.5*[w h], 'auto');
		...
		if frame > 1
            for i=1:size(search_size, 2)
                ...
                responsef(:,:,i) = sum(hf_num .* zf, 3) ./ (hf_den + lambda);
            ...
            szid = szid(1);
            responsef_sel = responsef(:,:,szid);
            interp_sz = floor(4*(target_sz * (1 + padding))*search_size(i));
            responsef_sel_interp = resizeDFT2(responsef_sel, interp_sz);
            response_sel = real(ifft2(responsef_sel_interp));
            response_sel = imresize(response_sel,[w,h]);
